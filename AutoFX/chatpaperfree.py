...
from configs import proxy_gpt, temp_dir

proxy = {'http':proxy_gpt['http'], 
         'https':proxy_gpt['https']}
...
class Reader:
    ...
    def summary_with_chat(self, paper_list):
        ...
        for paper_index, paper in enumerate(paper_list):
            ...
            try:
                ...
                chat_summary_text = 'Some Error for summary'
                utoken1, ctoken1, ttoken1 = 0, 0, 0
            ...
            if method_key != '':
                ...
                chat_method_text = 'Some Error for method(比如是不是没有Methods这个章节)'
            ...
            try:
                ...
                print(Exception)
                chat_conclusion_text = 'Some Error for conclusion'
            ...
            with open(os.path.join(temp_dir, 'output.md'), "w", encoding="utf8") as f:
               f.write(md_text)
            return markdown.markdown(md_text), pos_count
            return mistune.markdown(md_text), pos_count
            times = 0
            while True:
                prompt = f'''Consider the following Markdown text:

    {md_text}

    Now, your task is to refine the formatting without altering the hierarchy of the titles. Specifically, adjust the indentation of the lists to enhance the overall document structure. Ensure that the content remains unaltered. Your assistance is greatly appreciated!
    '''
                res_md_text = self.chatPaper.ask_one_shot(prompt)
                if self.check_answer(prompt, res_md_text):
                    flag = True
                else:
                    times += 1
                    sleep(5)
            ...
            with open(os.path.join(temp_dir, 'output.md'), "w", encoding="utf8") as f:
               f.write(res_md_text)
            return mistune.markdown(res_md_text), pos_count
    ...
    def chat_conclusion(self, text):
        ...
        while True:
            ...
            if times == 1: default_result = result
            if self.check_answer("This is the <summary> and <conclusion> part of an English literature, where <summary> you have already summarized, but <conclusion> part, I need your help to summarize the following questions:"
            + clip_text + content, result[0]):
                flag = True
            else:
                flag = False
                self.chatPaper.conversation['chatConclusion'].pop()
            if times > 3 or flag:
                break
            print("Try times ", times, result, flag)
        if flag == False:
    ...
    def chat_method(self, text):
        ...
        while True:
            ...
            if times == 1: default_result = result
            if self.check_answer(str(
                "This is the <summary> and <Method> part of an English document, where <summary> you have summarized, but the <Methods> part, I need your help to read and summarize the following questions."
                + clip_text) + content, result[0]):
                flag = True
            else:
                flag = False
                self.chatPaper.conversation['chatMethod'].pop()
            print("Try times ", times, result, flag)
            if times > 3 or flag:
                break
        if flag == False:
    ...
    def chat_summary(self, text):
        ...
        while True:
            ...
            if self.check_answer(str(
                ...
                self.chatPaper.conversation['chatSummary'].pop()
                flag = False
            print("Try times ", times, result, flag)
            if times > 3 or flag:
                break
    ...
    def check_answer(self, prompt, answer):
        print("Checking answer...")
        check_prompt = (
            "The large model has generated the following results. Please carefully assess the information and decide if it appears reasonable.\n\n"
            "Prompt:\n" + prompt + "\n\n"
            "Model's Answer:\n" + answer + "\n\n"
            "Do you find the model's answer reasonable? Reply with Yes or No."
        )
        result = self.chatPaper.ask_one_shot(check_prompt)
        # print(result)
        # 根据模型回答进行判断
