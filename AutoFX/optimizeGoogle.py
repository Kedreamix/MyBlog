...
class chatPaper:
    ...
    def __init__(
        ...
        if not self.check_api_available():
            raise "No available API key! Try again later"
        else:
            print("Using API key: ", self.get_api_key())
    ...
    def ask_one_shot(self, prompt: str):
        """
        Ask without conversation history
        """
        generation_config = {
            "temperature": 0.9,
            "top_p": 1,
    ...
    def check_api_available(self):
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                print(response.json())
                print("API is not available, response status code: " ,response.status_code)
            if flag or times > 5:
                break
